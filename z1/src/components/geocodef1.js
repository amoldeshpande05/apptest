...
function Geocode() {
  ...
  const [errstate, seterrstate] = useState(false); // err txt in component
  const [errtext, seterrtext] = useState('A valid value is required');
  const [buttonstate, setbuttonstate] = useState(false);
  const [err1status, seterr1status] = useState(false);  // err pop-ups
    ...
    if (location) {
        ...
        try{
          ...
          if(JSON.stringify(result.items) === '[]'){
            //alert('Invalid input. Input should not contains numbers or special characters');
             seterr3status(true);
             setGeocodeobj({});
          }
      ...
      // alert('Location input is empty. Please fill a valid Location');
      seterr1status(true);
      setLoading(false);
    }
  };
